...
class ParkingViewModel(
    ...
    fun handleReviewDeletion(parking: Parking = selectedParking.value!!, newScore: Double) {
        parking.avgScore = if (parking.nbReviews >= 2) {
            ((parking.nbReviews * parking.avgScore) - newScore) / (parking.nbReviews - 1)
        } else {
            0.0
        }
        parking.nbReviews -= 1
    ...
    fun handleNewReview(parking: Parking = selectedParking.value!!, newScore: Double) {
        parking.avgScore = (100 * ((parking.avgScore * parking.nbReviews) + newScore) / (parking.nbReviews + 1))
            .toInt() / 100.00
        parking.nbReviews += 1
        parkingRepository.updateParking(
            ...
            onSuccess = {
                val tile = Tile.getTileFromPoint(parking.location.center)
                tilesToParking.value[tile]?.let { parkingList ->
                    tilesToParking.value[tile] = parkingList.map { p -> if (p.uid == parking.uid) parking else p }
                }
            },
            onFailure = {}
        )
    ...
    fun handleReviewUpdate(parking: Parking = selectedParking.value!!, newScore: Double, oldScore: Double) {
